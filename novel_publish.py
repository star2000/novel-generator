...
class FanQieNovelAuthorClient:
    ...
    def _request(self, method, url, **kwargs):
        resp = requests.request(
            method, urljoin(self.BASE_URL, url), cookies=self.cookies, **kwargs
        )
    ...
    def secsdk_csrf_token(self):
        ...
        resp = requests.head(
            'https://fanqienovel.com/ttwid/check/',
    ...
    def check_volume_name(volume_name: str):
        assert re.match(r'第[零一二三四五六七八九十百千万亿]+[卷]：.+', volume_name), (
            f'{volume_name} 卷名格式错误'
        )
    ...
    def new_article(self, book_id: str) -> NewArticle:
        """新建文章"""
        return self._post(
            'article/new_article/v0', {'book_id': book_id, 'need_reuse': 1}
        )
...
if __name__ == '__main__':
    ...
    for book in client.get_book_list():
        ...
        for part_dir in u.sorted_subdirs(novel_dir):
            ...
            volume_name = part_dir.name.replace(str(part_num), part_cnnum, 1).replace(
                '-', '：', 1
            )
            ...
            for chapter_dir in u.sorted_subdirs(part_dir):
                ...
                article_content = (chapter_dir / '正文.txt').read_text(encoding='utf-8')
                is_rename = False
